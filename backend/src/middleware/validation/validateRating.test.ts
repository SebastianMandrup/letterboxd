...
describe('validateRating', () => {
    ...
    it('throws MuoError if rating is not a number', () => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        expect(() => validateRating(null as any)).toThrow(MuoError);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        expect(() => validateRating(undefined as any)).toThrow(MuoError);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        expect(() => validateRating('5' as any)).toThrow(MuoError);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        expect(() => validateRating(NaN as any)).toThrow('Rating must be a number');
    });

    it('throws MuoError if rating is not in increments of 0.5', () => {
        expect(() => validateRating(1.3)).toThrow(MuoError);
        expect(() => validateRating(2.7)).toThrow('Rating must be in increments of 0.5');
        expect(() => validateRating(4.1)).toThrow(MuoError);
    });

    it('throws MuoError if rating is below 0', () => {
        expect(() => validateRating(-0.5)).toThrow(MuoError);
        expect(() => validateRating(-1)).toThrow('Rating must be between 0 and 5');
    });

    it('throws MuoError if rating is above 5', () => {
        expect(() => validateRating(5.5)).toThrow(MuoError);
