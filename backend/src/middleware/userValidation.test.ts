...
describe('validateUserCreation middleware', () => {
    ...
    beforeEach(() => {
        ...
        jest.resetAllMocks();
        (validation.default as jest.Mock).mockImplementation((u: string) => `valid-${u}`);
        (passwordValidation.default as jest.Mock).mockImplementation((p: string) => `valid-${p}`);
        (emailValidation.default as jest.Mock).mockImplementation((e: string) => `valid-${e}`);
    });
