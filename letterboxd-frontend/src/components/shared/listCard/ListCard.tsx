	...
	return (
		<article className={styles.listCard}>
			...
			<section className={styles.listInfo}>
				<a className={styles.listName + (large ? ` ${styles.large}` : "")} href={`/lists/${getSlug(list.name)}`}>{list.name}</a>
				<a className={styles.listAuthor + (large ? ` ${styles.large}` : "")} href={`/users/${getSlug(list.user.username)}`}>Created by
